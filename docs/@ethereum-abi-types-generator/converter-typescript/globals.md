[**@ethereum-abi-types-generator/converter-typescript v2.0.0**](README.md) • **Docs**

***

[Documentation v2.0.0](../../packages.md) / @ethereum-abi-types-generator/converter-typescript

# @ethereum-abi-types-generator/converter-typescript

## Classes

- [ClassGenerator](classes/ClassGenerator.md)
- [Generator](classes/Generator.md)
- [InternalInterfaceV5](classes/InternalInterfaceV5.md)
- [InternalInterfaceV6](classes/InternalInterfaceV6.md)
- [TypingsGenerator](classes/TypingsGenerator.md)

## Interfaces

- [AbiItemModel](interfaces/AbiItemModel.md)
- [AbiModel](interfaces/AbiModel.md)
- [ContractSendMethod](interfaces/ContractSendMethod.md)
- [ContractVersionV4](interfaces/ContractVersionV4.md)
- [ContractVersionV5](interfaces/ContractVersionV5.md)
- [ContractVersionV6](interfaces/ContractVersionV6.md)
- [DeployOptions](interfaces/DeployOptions.md)
- [EstimateGasOptions](interfaces/EstimateGasOptions.md)
- [EthersContractV4](interfaces/EthersContractV4.md)
- [EthersContractV5](interfaces/EthersContractV5.md)
- [EthersContractV6](interfaces/EthersContractV6.md)
- [EventData](interfaces/EventData.md)
- [EventOptions](interfaces/EventOptions.md)
- [EventResponse](interfaces/EventResponse.md)
- [Log](interfaces/Log.md)
- [Options](interfaces/Options.md)
- [PromiEvent](interfaces/PromiEvent.md)
- [SendOptions](interfaces/SendOptions.md)
- [TransactionReceipt](interfaces/TransactionReceipt.md)

## Type Aliases

- [ContractFunctionV5](type-aliases/ContractFunctionV5.md)
- [ContractFunctionV6](type-aliases/ContractFunctionV6.md)
- [EthersContractContextV4](type-aliases/EthersContractContextV4.md)
- [EthersContractContextV5](type-aliases/EthersContractContextV5.md)
- [EthersContractContextV6](type-aliases/EthersContractContextV6.md)
- [Web3ContractContext](type-aliases/Web3ContractContext.md)
