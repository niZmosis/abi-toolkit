...
import { defaultConfigArgs, defaultConfigFileName } from './defaults.utils'
import { loadConfigFile } from './files.utils'
import { languageTypes } from './language.utils'
import { libraryTypes, frameworkTypes } from './library.utils'
import { Logger } from './logger.utils'
import { removeAllWhiteSpace } from './strings.utils'
...
export async function getProgramArguments(
  ...
  const typingsOutputDir =
    args.typingsOutputDir ||
    config?.typingsOutputDir ||
    defaultConfigArgs.typingsOutputDir
  ...
  const typingsOutputFileName = removeAllWhiteSpace(
    args.typingsOutputFileName ||
      config?.typingsOutputFileName ||
      defaultConfigArgs.typingsOutputFileName,
  )
  const typingsOutputFileSuffix = removeAllWhiteSpace(
    args.typingsOutputFileSuffix ||
      config?.typingsOutputFileSuffix ||
      defaultConfigArgs.typingsOutputFileSuffix,
  )
  const typingsPrefixTypes =
    args.typingsPrefixTypes ||
    config?.typingsPrefixTypes ||
    defaultConfigArgs.typingsPrefixTypes
  ...
  const classOutputFileName = removeAllWhiteSpace(
    args.classOutputFileName ||
      config?.classOutputFileName ||
      defaultConfigArgs.classOutputFileName,
  )
  const classOutputFileSuffix = removeAllWhiteSpace(
    args.classOutputFileSuffix ||
      config?.classOutputFileSuffix ||
      defaultConfigArgs.classOutputFileSuffix,
  )
